        ...
        moviesLoader.loadMovies { [weak self] result in
            DispatchQueue.main.async {
                ...
                case .success(let mostPopularMovies):
                    self.movies = mostPopularMovies.items // saving a movie into ou new variable
                    self.delegate?.didLoadDataFromServer() // informing on load complete
                case .failure(let error):
                    self.delegate?.didFailToLoadData(with: error) //Telling MovieQuizController about our mistake
                }
            }
        }
    }
